...
export function PaperCard({ entry }: PaperCardProps) {
    ...
    return (
        <Link
            ...
            className="block rounded-xl border border-neutral-200 bg-white p-5 shadow-sm transition hover:border-indigo-400 hover:shadow-md"
        >
            <h3 className="font-semibold text-neutral-900 leading-snug line-clamp-2">{entry.title}</h3>

            {entry.summary && (
                <p className="mt-1.5 text-sm text-neutral-600 line-clamp-2">{entry.summary}</p>
            )}
